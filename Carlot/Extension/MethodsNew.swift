...
//    let vc = self.storyboard?.instantiateViewController(withIdentifier: "CustomPopUpVC") as! CustomPopUpVC
    ...
    func showCustomPopupView(altMsg:String, alerttitle:String, alertimg:UIImage,cancelHide:Bool? = false, OkAction:@escaping ()->Void){
          ...
          vc.alertTitle=alerttitle
          vc.alertImage=alertimg
         vc.cancelHide = cancelHide ?? false
          self.present(vc, animated: true) {
              print("pop")
          }
          OKNewCallBack = { () in
...
//    }
   ...
   func changeStatusBarColor() {
    ...
    func log_Out_from_App(){
        ...
        if !isfind{
            let vc = LoginVC.instance(.main) as! LoginVC
...
//        }
    ...
    func checkEmptyString(str:String?)->String
    {
        ...
        func imagePick(objVC:UIViewController){
            ...
            let alertController  = UIAlertController(title:"Select option",message:"",preferredStyle:UIAlertController.Style.actionSheet)
    ...
    func VideoPick(objVC:UIViewController){
        ...
        let cameraAction = UIAlertAction(title: "Choose from the Camera", style: UIAlertAction.Style.default, handler:
            {Void in
                if(UIImagePickerController .isSourceTypeAvailable(UIImagePickerController.SourceType.camera))
                {
                    imagePicker.sourceType = UIImagePickerController.SourceType.camera
            ...
            alertController.addAction(cameraAction)
        let GalleryAction = UIAlertAction(title: "Choose from the Gallery", style: UIAlertAction.Style.default, handler:
            {Void in
                imagePicker.sourceType = UIImagePickerController.SourceType.photoLibrary
                imagePicker.allowsEditing = true
                imagePicker.mediaTypes = ["public.movie"]
    ...
    func DirectVideoPick(objVC:UIViewController){
            let imagePicker = UIImagePickerController()
            imagePicker.delegate = self
